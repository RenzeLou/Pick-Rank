...
sample_num_list = [9]  ## there are 5 kinds of negative instructions  ,2,3,4,5
...
def main():
    ...
    base_em,base_rg = base_results["EM"]["AVG"],base_results["RougeL"]["AVG"]
    base_em_rg = np.mean([base_em,base_rg])
    good_results,bad_results = dict(),dict()
    for loss_mix_ratio in loss_mix_ratio_list:
        for margin in margin_list:
            for func in neg_loss_type_list:
                for sample_num in sample_num_list:
                    for l in lr:
                        for e in epoch:
                            exp_name = exp_name_template.format(margin,sample_num,loss_mix_ratio,func,l,e)
                            file_name = os.path.join(exp_path,exp_name,"predict_results.json")
                            try:
                                em,rg = read_exp_results_v2(file_name)
                                em_rg_avg = np.mean([em,rg])
                            except FileNotFoundError:
                                print("warn: the exp '{}' is not ready!".format(exp_name))
                                continue
                            if em >= base_em and rg >= base_rg:
                                good_results[exp_name] = [em,rg,em_rg_avg]
                            else:
                                bad_results[exp_name] = [em,rg,em_rg_avg]
    
    key = lambda x:x[1][-1]
    
    good_results = dict(sorted(good_results.items(),key=key,reverse=True))
    bad_results = dict(sorted(bad_results.items(),key=key,reverse=True))
    
    good_results["base"] = [base_em,base_rg,base_em_rg]
    bad_results["base"] = [base_em,base_rg,base_em_rg]
    
    # print the global best found
    print("\nHere are {} good result:".format(len(good_results)))
    for k,v in good_results.items():
        print("{}: {}".format(k,v))
    print("\nHere are {} bad result:".format(len(bad_results)))
    for k,v in bad_results.items():
        print("{}: {}".format(k,v))
        
    good_name = os.path.join(out_path,"good_results.json")
    bad_name = os.path.join(out_path,"bad_results.json")
    os.makedirs(out_path,exist_ok=True)
    with open(good_name,"w",encoding="utf-8") as f:
        json.dump(good_results,f,indent=2,sort_keys=True)
    with open(bad_name,"w",encoding="utf-8") as f:
        json.dump(bad_results,f,indent=2,sort_keys=True)
        
    print("\nfile dump successfully:\n{}, {}".format(good_name,bad_name))
