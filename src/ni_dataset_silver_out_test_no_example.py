...
'''
different with v1, combine the pos instructions with the neg samples
thus we can calculate the constractive loss between the pos and neg
'''
...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_eval_task=None,
                 sample_num_neg=None, sample_num_pos=None,training_ins_num=None,add_task_definition_train=True,
                 add_task_definition_test=True,neg_loss_type=None,null_loss_type=None,out_loss_type=None,
                 train_add_silver=False,test_add_silver=False,test_no_examples=False,
                 **kwargs):
        ...
        self.add_task_definition_test=add_task_definition_test
        self.add_task_definition_train=add_task_definition_train
        ...
        self.train_add_silver=train_add_silver
        self.test_add_silver=test_add_silver
        self.test_no_examples=test_no_examples
        ...
        with open(path, encoding="utf-8") as split_f:
            for line in split_f:
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    # assert len(neg_def) == 9, "here the length is {} for {}".format(len(neg_def),task_name)
                    ...
                    sample_num_neg = min(len(neg_def),sample_num_neg)
                    neg_def_sampled = random.sample(neg_def,sample_num_neg)
                    ...
                    ## TODO: if there is multiple pos definitions (different instance has different num of pos samples), then I need to add a 'indicators'
                    # if self.config.null_def_cons and subset == "train":
                    #     all_def = ori_def + null_def
                    #     assert len(all_def) == 2
                    # else:
                    #     all_def = ori_def + pos_def_sampled +neg_def_sampled
                    # assert self.config.neg_loss_type is not None or self.config.null_loss_type is not None, "at least one additional loss!"
                    null_def = []
                    negative_def = []
                    if self.config.null_loss_type is not None and subset == "train":
                        null_def = ["NULL."]
                    if self.config.neg_loss_type is not None and subset == "train":
                        negative_def = neg_def_sampled + pos_def_sampled
                    # concatnate ori def, null def and neg def
                    all_def = ori_def + null_def + negative_def
                    if subset == "test" or subset == "dev":
                        ...
                        # so, we use them here
                        if not self.config.add_task_definition_test:
                            all_def = [""]
                        instances = all_instances[:100]
                    else:
                        if not self.config.add_task_definition_train:
                            all_def = [""]
                    ...
                    # concat each instance with multiple instructions (pos / neg)
                    # idx = -1
                    ...
                    # randomly select the pos examples
                    pos_exps_ori = task_data.pop("Positive Examples")
                    pos_exps_ori = random.sample(pos_exps_ori,min(len(pos_exps_ori),2))
                    # print(pos_exps)
                    # raise RuntimeError
                    for idx, instance in enumerate(instances):
                        # for _idx,df in enumerate(all_def):
                        # idx += 1
                        example = task_data.copy()
                        example["id"] = instance["id"]
                        # decide whether add pos examples
                        pos_exps = copy.deepcopy(pos_exps_ori)
                        if subset == "train":
                            # for training set
                            if self.config.train_add_silver:
                                new_pos_exps = []
                                for exp in pos_exps:
                                    exp["output"] = exp.pop("output_silver")
                                    new_pos_exps.append(exp)
                                pos_exps = new_pos_exps
                            else:
                                new_pos_exps = []
                                for exp in pos_exps:
                                    exp.pop("output_silver")
                                    new_pos_exps.append(exp)
                                pos_exps = new_pos_exps
                        else:
                            # for test set
                            if self.config.test_add_silver:
                                new_pos_exps = []
                                for exp in pos_exps:
                                    exp["output"] = exp.pop("output_silver")
                                    new_pos_exps.append(exp)
                                ...
                                for exp in pos_exps:
                                    ...
                                    new_pos_exps.append(exp)
                                pos_exps = new_pos_exps
                            # do not add any examples (only def for test)
                            if self.config.test_no_examples:
                                pos_exps = []
                        ## reset the pos examples for this instance
                        example["Positive Examples"] = pos_exps
                        
                        if subset == "test" or subset == "dev" or self.config.out_loss_type is None:
                            ## note that if out_loss_type is none, means we don not do the output constrain,
                            ## there is no need to add "neg_output" in the instance
                            instance["neg_output"] = []
                        # if subset == "train" and len(instance["neg_output"])==0 \
                        #                     and self.config.current_training_ins_num >= self.config.training_ins_num:
                            # for those instances without neg_output in training set, skip it
                            # continue
                        example["Instance"] = instance  ## instance is a dict containing input, output, id, etc.
                        example["Definition"] = all_def
