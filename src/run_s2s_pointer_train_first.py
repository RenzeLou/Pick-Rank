...
from ni_collator_pointer_train_first import DataCollatorForNI
# from ni_trainer_augment_v2 import NITrainer, DenserEvalCallback
from ni_trainer_pointer_train_first import NITrainer, DenserEvalCallback
...
# from modeling_t5_pointer_multi_sample import T5ForConditionalGeneration_neg, T5Stack
...
class NITrainingArguments(Seq2SeqTrainingArguments):
    ...
    pos_loss_type: str = field(
        default=None, metadata={"help": "The loss function for repeated definition."}
    )
    ...
    del_detach: Optional[bool] = field(
        default=False  ###
    )
    ori_anchor: Optional[bool] = field(
        default=False  ###
    )
    ...
    structured_anchor: Optional[bool] = field(
        default=False  ###
    )
    null_anchor: Optional[bool] = field(
        default=False  ###
    )
...
def main():
    ...
    tokenizer.add_special_tokens({"additional_special_tokens": ["[REPEAT]","[DEL]"]})
    
    assert training_args.del_detach, "the del_detach can have a better performance, if you really want to use other methods, please change the code"
    
    if training_args.ori_anchor:
        # (rep vs. ori) + (ori vs. del) + (ori vs. null) 
        from modeling_t5_pointer_train_first_ori_anchor import T5ForConditionalGeneration_neg, T5Stack
    elif training_args.structured_anchor:
        # (rep vs. ori) + (ori vs. del) + (del vs. null)
        from modeling_t5_pointer_train_first_structured_anchor import T5ForConditionalGeneration_neg, T5Stack
    elif training_args.null_anchor:
        # (rep vs. null) + (ori vs. null) + (del vs. null)
        from modeling_t5_pointer_train_first_null_anchor import T5ForConditionalGeneration_neg, T5Stack
    else:
        from modeling_t5_pointer_train_first import T5ForConditionalGeneration_neg, T5Stack
    # if training_args.del_detach:
    #     from modeling_t5_pointer_multi_sample_del_detach import T5ForConditionalGeneration_neg, T5Stack
    # elif training_args.ori_anchor:
    #     from modeling_t5_pointer_multi_sample_ori_anchor import T5ForConditionalGeneration_neg, T5Stack
    # else:
    #     from modeling_t5_pointer_multi_sample import T5ForConditionalGeneration_neg, T5Stack
    ...
    if not training_args.base:
        ...
        if training_args.add_pointer_encoder:
            if training_args.pointer_encoder_checkpoint is not None:
                ...
                logger.info("**"*10 + "=> no pointer encoder checkpoint, copy the encoder of the main model" + "**"*10)
    ...
    trainer.init_hyper(pos_neg_ratio=training_args.pos_neg_ratio,
                       margin_pos=training_args.margin_pos,
