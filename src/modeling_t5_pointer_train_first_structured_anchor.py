    ...
    def forward(
        ...
        if encoder_outputs_ori is None and encoder_outputs_rep is None and encoder_outputs_del is None and encoder_outputs_null is None:
            if not base:
                ...
                if not test_phase and (neg_loss_type is not None or null_loss_type is not None):
                    def_del_list = []
            ...
            # prepare ori def + x
            if (not test_phase and (pos_loss_type is not None or neg_loss_type is not None or null_loss_type or not main_loss_on_rep)) or (test_phase and not predict_on_rep):
