...
from ni_collator_memory_test import DataCollatorForNI
# from ni_trainer_augment_v2 import NITrainer, DenserEvalCallback
from ni_trainer_memory_test import NITrainer, DenserEvalCallback
...
from modeling_t5_memory_test import T5ForConditionalGeneration_neg
...
class NITrainingArguments(Seq2SeqTrainingArguments):
    ...
    add_hidden_projector_test: Optional[bool] = field(
        default=False  ###
    )
...
def main():
    ...
    ## TODO: test whether the projector can impact the performance
    if training_args.add_hidden_projector_test:
        model.add_hidden_projector_test(config.d_model,config.d_model)
    else:
        model.add_hidden_projector_test(None,None)
    ...
    trainer.init_hyper(pos_neg_ratio=training_args.pos_neg_ratio,margin_pos=training_args.margin_pos,
                       ...
                       pooling = training_args.pooling,reverse=training_args.reverse,
