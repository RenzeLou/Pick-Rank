...
from ni_collator_pointer_multi_sample import DataCollatorForNI
# from ni_trainer_augment_v2 import NITrainer, DenserEvalCallback
from ni_trainer_pointer_multi_sample import NITrainer, DenserEvalCallback
...
def main():
    ...
    raw_datasets = load_dataset( 
        "src/ni_dataset_pointer_multi_sample.py", 
    ...
    if not training_args.base:
        ...
        if training_args.add_pointer_encoder:
            ...
            # pointer_encoder = T5Stack.from_pretrained(
            #     model_args.model_name_or_path,
            #     from_tf=bool(".ckpt" in model_args.model_name_or_path),
            ...
            #     revision=model_args.model_revision,
            #     use_auth_token=True if model_args.use_auth_token else None,
            # )  # TODO: check if the pretrained parameters of pointer_encoder are the same as the pretrained parameters of model.encoder
            # model.pointer_encoder.resize_token_embeddings(len(tokenizer))
            # test_1 = list(pointer_encoder.named_parameters())[0]
            # test_2 = list(model.encoder.named_parameters())[0]
