...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_eval_task=None,
                 ...
                 add_task_definition_test=True,neg_loss_type=None,null_loss_type=None,out_loss_type=None,words_num=1,
                 **kwargs):
        ...
        with open(path, encoding="utf-8") as split_f:
            for line in split_f:
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    ''' to test whether a few random words can be used as negative examples'''
                    # randomly select several words from the definition
                    definition = (" ".join(task_data["Definition"])).strip()
                    all_words = definition.split(" ")
                    choice = random.choice(list(range(len(all_words)-self.config.words_num+1)))
                    neg_def = [" ".join(all_words[choice:choice+self.config.words_num])]  # reset the neg def
