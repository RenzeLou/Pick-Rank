...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_eval_task=None, sample_num_neg=None, sample_num_pos=None,**kwargs):
        ...
        with open(path, encoding="utf-8") as split_f:
            for line in split_f:
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    all_examples = copy.deepcopy(task_data["Positive Examples"])
                    ...
                    if subset == "test":
                        ...
                        # all_def = pos_def_sampled + neg_def_sampled
                    ...
                    task_id = instances[0]["id"].split("-")[0]
                    # TODO: in this experiment, we will use the examples instead of instances
                    # concat each instance with multiple instructions (pos / neg)
                    for idx, instance in enumerate(all_examples):
                        for _,df in enumerate(all_def):
                            example = task_data.copy()
                            example["id"] = task_id + "-" + str(idx)
