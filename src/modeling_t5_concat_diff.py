...
class T5Attention_k_v_sep(nn.Module):
    ...
    def forward(
        ...
        key_length = real_seq_length if key_states is None else key_states.shape[1]
        value_length = real_seq_length if value_states is None else value_states.shape[1]
        assert key_length == value_length, "key and value must have the same number of elements."
        ...
        key_states = project(  ## renze: change this
            hidden_states, self.k, key_states, past_key_value[0] if past_key_value is not None else None
        )
        value_states = project(
            hidden_states, self.v, value_states, past_key_value[1] if past_key_value is not None else None
        )
...
class T5LayerSelfAttention_k_v_sep(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = T5Attention_k_v_sep(config, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5LayerCrossAttention_k_v_sep(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.EncDecAttention = T5Attention_k_v_sep(config, has_relative_attention_bias=False)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block_k_v_sep(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.layer = nn.ModuleList()
        self.layer.append(T5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias))
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention_k_v_sep(config))
    ...
    def forward(
        ...
        position_bias=None,
        encoder_hidden_states=None,
        encoder_hidden_states_ori=None,  ## this is the original encoder hidden states (i.e., no additional representation concated)
        encoder_attention_mask=None,
        encoder_decoder_position_bias=None,
        layer_head_mask=None,
        cross_attn_layer_head_mask=None,
        ...
        if past_key_value is not None:
            ...
            self_attn_past_key_value = past_key_value[:2]
            cross_attn_past_key_value = past_key_value[2:]
            # self_attn_past_value = past_value[:2]
            # cross_attn_past_value = past_value[2:]
        else:
            self_attn_past_key_value, cross_attn_past_key_value = None, None
            # self_attn_past_value, cross_attn_past_value = None, None
        ...
        # self_attention_outputs_ori = self.layer[0](
        #     hidden_states,
        #     attention_mask=attention_mask,
        #     position_bias=position_bias,
        #     layer_head_mask=layer_head_mask,
        #     past_key_value=self_attn_past_value,
        #     use_cache=use_cache,
        #     output_attentions=output_attentions,
        # )
        # hidden_states_ori, present_value_state = self_attention_outputs_ori[:2]
        # self_attention_outputs_ori = self_attention_outputs_ori[2:]  # Keep self-attention outputs and relative position weights
        ...
        if hidden_states.dtype == torch.float16 and torch.isinf(hidden_states).any():
            clamp_value = torch.finfo(hidden_states.dtype).max - 1000
            hidden_states = torch.clamp(hidden_states, min=-clamp_value, max=clamp_value)
            # clamp_value_ori = torch.finfo(hidden_states_ori.dtype).max - 1000
            # hidden_states_ori = torch.clamp(hidden_states_ori, min=-clamp_value_ori, max=clamp_value_ori)
        ...
        if do_cross_attention:
            ...
            cross_attention_outputs = self.layer[1](
                ...
                value_states=encoder_hidden_states_ori,
                attention_mask=encoder_attention_mask,
                position_bias=encoder_decoder_position_bias,
                layer_head_mask=cross_attn_layer_head_mask,
                past_key_value=cross_attn_past_key_value,
...
class T5Stack_k_v_sep(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        super().__init__(config)
        ...
        self.block = nn.ModuleList(
            [T5Block_k_v_sep(config, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
        )
        self.final_layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
    ...
    def forward(
        ...
        attention_mask=None,
        encoder_hidden_states=None,
        encoder_hidden_states_ori=None,
        encoder_attention_mask=None,
        inputs_embeds=None,
        head_mask=None,
        cross_attn_head_mask=None,
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.model_parallel:
                ...
                if encoder_hidden_states is not None:
                    encoder_hidden_states = encoder_hidden_states.to(hidden_states.device)
                if encoder_hidden_states_ori is not None:
                    encoder_hidden_states_ori = encoder_hidden_states_ori.to(hidden_states.device)
                if encoder_extended_attention_mask is not None:
                    encoder_extended_attention_mask = encoder_extended_attention_mask.to(hidden_states.device)
                if encoder_decoder_position_bias is not None:
                    encoder_decoder_position_bias = encoder_decoder_position_bias.to(hidden_states.device)
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(
                    ...
                    position_bias=position_bias,
                    encoder_hidden_states=encoder_hidden_states,
                    encoder_hidden_states_ori=encoder_hidden_states_ori,
                    encoder_attention_mask=encoder_extended_attention_mask,
                    encoder_decoder_position_bias=encoder_decoder_position_bias,
                    layer_head_mask=layer_head_mask,
                    cross_attn_layer_head_mask=cross_attn_layer_head_mask,
...
class T5ForConditionalGeneration_neg(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = T5Stack_k_v_sep(decoder_config, self.shared)
    ...
    def forward(
        ...
        neg_num = len(def_input_ids_list) - 1
        assert neg_num == 1
        ...
        input_ids = torch.cat(input_ids_list,dim=0)  ## [batch_size, max_seq_len]
        attention_mask = torch.cat(attention_mask_list,dim=0)
        def_input_ids = torch.cat([def_input_ids_list[1]],dim=0)  ## [batch_size , max_seq_len]
        def_attention_mask = torch.cat([def_attention_mask_list[1]],dim=0)
        ...
        if encoder_outputs is None:
            ...
            hidden_states = encoder_outputs[0]  # [batch_size, max_seq_len, hidden_size]
            ...
            if neg_num > 0:
                ...
                def_hidden_states = def_encoder_outputs[0]  # [batch_size, max_seq_len, hidden_size]
                ...
                def_hidden_pooled = self.pooling_hidden(def_hidden_states,def_attention_mask,pooling_memory) # [batch_size , hidden_size]
                ...
                # weights = self.get_att_weights(def_hidden_states,def_attention_mask,batch_szie,neg_num,pooling_att,reverse=reverse,neg_len=neg_len)  # [batch_size,neg_num]
                ...
                batched_memories = def_hidden_pooled  # [batch_size,hidden_size]
                ...
                # instead of sirectly concat, calculate the association between the sentence-level rep and the ori_def rep 
            ...
            if test_phase:
                ...
                encoder_outputs_saved_ori = [hidden_states_ori]
            ...
            hidden_states_ori = encoder_outputs_ori[0]
            attention_mask_ori = attention_mask[:batch_szie,:]
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_hidden_states_ori=hidden_states_ori,
