...
from ni_collator_def_segmentation import DataCollatorForNI
# from ni_trainer_augment_v2 import NITrainer, DenserEvalCallback
from ni_trainer_def_segmentation import NITrainer, DenserEvalCallback
...
from modeling_t5_def_segmentation import T5ForConditionalGeneration_neg
...
class NITrainingArguments(Seq2SeqTrainingArguments):
    ...
    v_projector_dim: Optional[int] = field(
        default = 768,   ###
    )
    k_projector_dim: Optional[int] = field(
        default = 768,   ###
    )
    add_v_projector: Optional[bool] = field(
        default=False,   ###
    )
    add_k_projector: Optional[bool] = field(
        default=False  ###
    )
    ...
    diff: Optional[bool] = field(
        default=False,  ### calculate the association between ori_def rep and sentence-level rep, instead of concat
    )
...
def main():
    ...
    raw_datasets = load_dataset( 
        "src/ni_dataset_def_segmentation.py", 
    ...
    if training_args.add_k_projector:
        model.add_k_projector(config.d_model,training_args.k_projector_dim)
        model.add_q_projector(hidden_dim=training_args.k_projector_dim)
    else:
        model.add_k_projector(None,None)
        model.add_q_projector(hidden_dim=config.d_model)
    
    if training_args.add_v_projector:
        model.add_v_projector(config.d_model,training_args.v_projector_dim)
        model.add_hidden_projector(config.d_model+training_args.v_projector_dim, config.d_model)
    else:
        model.add_v_projector(None,None)
        model.add_hidden_projector(config.d_model * 2, config.d_model)
    ...
    trainer.init_hyper(pos_neg_ratio=training_args.pos_neg_ratio,margin_pos=training_args.margin_pos,
                       ...
                       reverse=training_args.reverse,lr_proj=training_args.lr_proj
                       )
