...
class T5ForConditionalGeneration_neg(T5PreTrainedModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            if neg_num > 0:
                ...
                # get the memory bank
                perturbation_memories = self.memory_bank(hidden_states, attention_mask,batch_szie,neg_num,pooling) # [batch_size * neg_num, max_seq_len ,memo_dim]
                memory_for_each_ins = []
                ...
                perturbation_memories = torch.cat(memory_for_each_ins,dim=0)  # [batch_size * neg_num, max_seq_len, memo_dim]
                ...
                weights = weights.reshape(batch_szie*neg_num,1,1).repeat(1,perturbation_memories.shape[1],perturbation_memories.shape[-1])  # [batch_size * neg_num, max_seq_len,memo_dim]
                
                # weighted sum memories
                perturbation_memories_weighted = torch.mul(perturbation_memories,weights)  # [batch_size * neg_num,max_seq_len,memo_dim]
                ...
                for m in memories_lis:  # [neg_num,max_seq_len,memo_dim]
                    memory = torch.sum(m,dim=0,keepdim=True) # [1,max_seq_len,memo_dim]
                    batched_memories_lis.append(memory)
                batched_memories = torch.cat(batched_memories_lis,dim=0)  # [batch_size,max_seq_len,memo_dim]
                ...
                batched_memories_expanded = batched_memories  # [batch_size, max_seq_len, memo_dim]
    ...
    def memory_bank(self, def_hidden_states, def_attention_mask, batch_size, neg_num,pooling="max"):
        ...
        def_hidden_pooled_lis = torch.split(def_hidden_states,[batch_size,neg_num*batch_size],dim=0)
        def_hidden_ori = def_hidden_pooled_lis[0]  # [batch_size, max_seq_len, hidden_size]
        def_hidden_ori = def_hidden_ori.repeat(neg_num,1,1) # [neg_num * batch_size,  max_seq_len, hidden_size]
        def_hidden_neg = def_hidden_pooled_lis[1]  # [neg_num * batch_size, max_seq_len,hidden_size]
        
        # compute the assosiation between the ori definition and the purturbated definition
        diff = def_hidden_ori - def_hidden_neg  # [neg_num * batch_size, max_seq_len, hidden_size]
        dot = torch.mul(def_hidden_ori,def_hidden_neg)  # [neg_num * batch_size, max_seq_len, hidden_size]
        association = torch.cat([def_hidden_neg,diff,dot],dim=-1)  # [neg_num * batch_size,  max_seq_len,3*hidden_size]
        
        # print("self.memory_projector",self.memory_projector)
        # print("association",association.shape)
        # exit()
        memories = self.memory_projector(association) if self.memory_projector is not None else association  # [neg_num * batch_size, max_seq_len, memory_dim]
    ...
    def add_hidden_projector(self,input_size,output_size,bias=True):
