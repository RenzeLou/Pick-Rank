...
class DataCollatorForNI:
    ...
    def __call__(self, batch, return_tensors=None):
        ...
        for instance in batch:
            ...
            # add indicators
            assert 'indicator' in instance, "This is the experiments about the negative instructions, where 'indicator' required. Please modify the ni_dataset.py to conquer!"
            make_int = lambda x:int(x)
            indicator = list(map(make_int,instance['indicator']))
            indicators.extend(indicator)

        assert len(indicators) == len(sources)  ## one batch size
        ...
        # add indicators
        # model_inputs["indicators"] = torch.tensor(indicators,dtype=torch.int64)
        model_inputs["indicators"] = indicators
