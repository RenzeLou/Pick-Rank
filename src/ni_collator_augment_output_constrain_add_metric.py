    ...
    def __call__(self, batch, return_tensors=None):
        ...
        if "output" in batch[0]["Instance"] and batch[0]["Instance"]["output"]:
            ...
            for ex in batch:
                ...
                # each instance in this batch may have different label space (e.g., [Yes,No], [true,false], [])
                lb_sp = ex["Instance"]["label_space"]
                lb_sp_len.append(len(lb_sp))
                lb_sp_lis_one.extend(lb_sp)
            ...
            # process label space, used for eval 
            if len(lb_sp_lis_one) > 0:
                with self.tokenizer.as_target_tokenizer():
                    all_lb_sp = self.tokenizer(
                        lb_sp_lis_one,
                        max_length=self.max_target_length,
                        padding=self.padding,
                        return_tensors=self.return_tensors,
                ...
                lb_sp_mask = all_lb_sp["attention_mask"].bool()
                all_lb_sp_ids = all_lb_sp["input_ids"].masked_fill(~lb_sp_mask, self.label_pad_token_id)  
                model_inputs["label_space_ids"] = all_lb_sp_ids
                model_inputs["label_space_ids_len"] = lb_sp_len
            else:
                model_inputs["label_space_ids"] = None
                model_inputs["label_space_ids_len"] = lb_sp_len
        else:
        ...
        # prepare decoder_input_ids (for label space)
        if self.model is not None and hasattr(self.model, "prepare_decoder_input_ids_from_labels"):     
            lb_sp_decoder_input_ids = self.model.prepare_decoder_input_ids_from_labels(labels=model_inputs["label_space_ids"]) if model_inputs["label_space_ids"] is not None else None
            model_inputs["label_space_decoder_input_ids"] = lb_sp_decoder_input_ids
