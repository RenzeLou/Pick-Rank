...
class T5ForConditionalGeneration_neg(T5PreTrainedModel):
    ...
    def forward(
        ...
        input_ids = torch.cat(input_ids_list,dim=0)  ## [batch_size, max_seq_len]
        attention_mask = torch.cat(attention_mask_list,dim=0)
        def_input_ids = torch.cat(def_input_ids_list[1:],dim=0) if neg_num > 0 else None ## [batch_size * neg_num, max_seq_len]
        def_attention_mask = torch.cat(def_attention_mask_list[1:],dim=0) if neg_num > 0 else None
