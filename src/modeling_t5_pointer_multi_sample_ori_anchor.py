    ...
    def forward(
        ...
        if encoder_outputs_ori is None and encoder_outputs_rep is None and encoder_outputs_del is None and encoder_outputs_null is None:
            if not base:
                ...
                # prepare null def ([DEL] + x)
                if not test_phase and null_loss_type is not None:
                    def_null_list = []
                    att_null_list = []  # containing a batch of samples 
                    for i in range(batch_size):
                        temp_x = x_tk_list[i]
                        att_null = torch.ones(temp_x.shape[0],temp_x.shape[1]+1, dtype=torch.float).to(temp_x.device)
                        def_null = torch.cat([del_tk,temp_x],dim=-1)
            ...
            # prepare ori def + x
            if (not test_phase and (pos_loss_type is not None or neg_loss_type is not None or not main_loss_on_rep)) or (test_phase and not predict_on_rep):
