...
class DataCollatorForNI:
    ...
    def __call__(self, batch, return_tensors=None):
        ...
        #     index_list = [index+t for t in range(batch_size)]
        ...
        # model_inputs["rep_tk"], model_inputs["del_tk"] = self.tokenizer.special_tokens_map["[REPEAT]"], self.tokenizer.special_tokens_map["[DEL]"]  # special token ids
        model_inputs["rep_tk"], model_inputs["del_tk"] = self.tokenizer.additional_special_tokens_ids[0], self.tokenizer.additional_special_tokens_ids[1] # special token ids
        model_inputs["rep_tk"], model_inputs["del_tk"] = torch.LongTensor([model_inputs["rep_tk"]]), torch.LongTensor([model_inputs["del_tk"]])
