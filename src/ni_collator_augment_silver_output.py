...
class DataCollatorForNI:
    ...
    silver_y: bool=False
    silver_x_y: bool=False
    shuffle_y: bool=False
    random_select_pos: bool=False
    ...
    def __call__(self, batch, return_tensors=None):
        ...
        for instance in batch:
            ...
            # TODO: I am not sure, perhaps need to split def and example
            source_list = []  ## a list stands for one instance containing both pos and neg instruction input
            for definition in definitions:
                # try to add positive examples.
                pos_examples = []
                example_input_list,example_output_list = [],[]
                # random select pos examples
                if self.random_select_pos:
                    selected_pos_examples = random.sample(instance["Positive Examples"],min(len(instance["Positive Examples"]),num_pos_examples))
                else:
                    selected_pos_examples = instance["Positive Examples"][:num_pos_examples]
                # choose to replace the ori pos examples with silver y
                for _, pos_example in enumerate(selected_pos_examples):
                    example_input, example_output = pos_example['input'], pos_example['output']
                    assert not self.silver_y or not self.silver_x_y, "can not be ture at the same time!"
                    if self.silver_y:
                        # use silver y
                        example_output = pos_example['output_silver']
                    elif self.silver_x_y:
                        # use silver (x,y) pairs
                        raise NotImplementedError("emmmm")
                    example_input_list.append(example_input)
                    example_output_list.append(example_output)
                # shuffle the y, to see whether the x,y correspondence can influence the results
                # random.shuffle(example_output_list)
                # since only have two examples, just reverse the list
                if num_pos_examples <=2:
                    example_output_list.reverse()
                else:
                    random.shuffle(example_output_list)
                for idx,(example_input, example_output) in enumerate(zip(example_input_list,example_output_list)):
                    pos_example_str = f" Positive Example {idx+1} -\n"
                    pos_example_str += f"Input: {example_input.strip()}"
                    if not pos_example_str[-1] in string.punctuation:
                        pos_example_str += "."
                    pos_example_str += "\n"
                    pos_example_str += f" Output: {example_output.strip()}"
                    if not pos_example_str[-1] in string.punctuation:
                        pos_example_str += "."
                    pos_example_str += "\n" 
