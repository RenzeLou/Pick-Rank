...
class T5ForConditionalGeneration_neg(T5PreTrainedModel):
    ...
    def forward(
        ...
        if input_ids_list is None and attention_mask_list is None and def_input_ids_list is None and def_attention_mask_list is None:
            ...
            pooling_att = self.gen_kwargs["pooling_att"]
            pooling_memory = self.gen_kwargs["pooling_memory"]
            ...
            encoder_outputs_def = self.gen_kwargs.get("encoder_outputs_def", None)
            pooling = self.gen_kwargs.get("pooling", "max")
            reverse = self.gen_kwargs.get("reverse", False)
            
            if len(input_ids_list) > 1:
                # when doing test, do not use the null def
                input_ids_list = [input_ids_list[0]]
                attention_mask_list = [attention_mask_list[0]]
        ...
        neg_num = len(def_input_ids_list)

        # Encode if needed (training, first prediction pass)
        # sample_num = len(def_input_ids_list)  ## should be 1 (ori) + 1 (null) + 5 (neg)
        pos_sample = input_ids_list[0]
        if len(pos_sample.shape) < 2:
            batch_szie = 1
        elif len(pos_sample.shape) == 2:
        ...
        # assert len(input_ids_list) == 1, "only use ori def for 'def + x'"
        null_num = len(input_ids_list) - 1
        ...
        # use batch forward instead of for-loop
        input_ids = torch.cat(input_ids_list,dim=0)  ## [batch_size * null_num, max_seq_len]
        attention_mask = torch.cat(attention_mask_list,dim=0)
        ...
        if encoder_outputs is None:
            ...
            hidden_states = encoder_outputs[0]  # [batch_size * (1+null_num), max_seq_len, hidden_size]
            ...
            if neg_num > 0:
                ...
                # perturbation_memories = self.memory_bank(def_hidden_states, def_attention_mask,batch_szie,neg_num,pooling_memory) # [batch_size * neg_num, memo_dim]
                # perturbation_memories = self.pooling_hidden(def_hidden_states,def_attention_mask,pooling_memory) # [batch_size * neg_num, memo_dim]
                # memory_for_each_ins = []
                ...
                # v_vecotrs = self.v_projector(perturbation_memories) if self.v_projector is not None else perturbation_memories  # [batch_size * neg_num, V_dim]
                
                # get the attention weights
                # neg_len = [l-1 for l in def_len]
                # weights = self.get_att_weights_linear(def_hidden_states,def_attention_mask,batch_szie,neg_num,pooling_att,reverse=reverse,neg_len=neg_len)  # [batch_size,neg_num]
                # weights = weights.reshape(batch_szie*neg_num,1).repeat(1,v_vecotrs.shape[-1])  # [batch_size * neg_num,memo_dim]
                
                # weighted sum memories
                # v_weighted = torch.mul(v_vecotrs,weights)  # [batch_size * neg_num,memo_dim]
                # memories_lis = torch.split(v_weighted,neg_num,dim=0)
                ...
                # instead of directly concat, calculate the association between the sentence-level rep and the ori_def rep 
                # ori_token_rep = self.token_projector(hidden_states_ori) if self.token_projector is not None else hidden_states_ori  # [batch_size, max_seq_len, hidden_size]
                # expand_sentence_rep = self.sentence_projector(batched_memories_expanded) if self.sentence_projector is not None else batched_memories_expanded  # [batch_size, max_seq_len, hidden_size]
                # diff = ori_token_rep - expand_sentence_rep  # [batch_size, max_seq_len, hidden_size]
                # dot = torch.mul(ori_token_rep,expand_sentence_rep)  # [batch_size, max_seq_len, hidden_size]
                # association = torch.cat([diff,dot],dim=-1)  # [batch_size, max_seq_len, hidden_size * 2]
                
                # hidden_states_mixed = self.hidden_projector(association) # [batch_size, max_seq_len, hidden_size]
            ...
            if test_phase:
                ...
                encoder_outputs_saved = [hidden_states]
                encoder_outputs_saved_def = [def_hidden_states]
                self.gen_kwargs["encoder_outputs"] = encoder_outputs_saved
                self.gen_kwargs["encoder_outputs_def"] = encoder_outputs_saved_def
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput):
            encoder_outputs = BaseModelOutput(
                last_hidden_state=encoder_outputs[0],
                hidden_states=encoder_outputs[1] if len(encoder_outputs) > 1 else None,
            ...
            hidden_states = encoder_outputs[0]
            def_hidden_states = encoder_outputs_def[0]
            attention_mask = attention_mask[:batch_szie,:]
        ...
        # Decode
        decoder_input_ids_for_null = decoder_input_ids.repeat(null_num+1,1)
        decoder_attention_mask_for_null = decoder_attention_mask.repeat(null_num+1,1) if decoder_attention_mask is not None else None
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids_for_null,
            attention_mask=decoder_attention_mask_for_null,
            inputs_embeds=decoder_inputs_embeds,
            past_key_values=past_key_values,
            encoder_hidden_states=hidden_states,
            encoder_attention_mask=attention_mask,
        ...
        lm_logits = self.lm_head(sequence_output)  ## [batch_size * (1+null_num), max_seq_length, vocab_size]
        lm_logits_lis = torch.split(lm_logits,batch_szie,dim=0)
        assert len(lm_logits_lis) == null_num + 1
        lm_logits_ori = lm_logits_lis[0]  # [batch_size , max_seq_len, vocab_size]
        lm_logits_null = torch.cat(lm_logits_lis[1:],dim=0) if null_num > 0 else None # [batch_size * null_num, max_seq_len, vocab_size]
        
        
        if def_hidden_states is not None and not test_phase:
            decoder_input_ids_for_def = decoder_input_ids.repeat(neg_num,1)
            decoder_attention_mask_for_def = decoder_attention_mask.repeat(neg_num,1) if decoder_attention_mask is not None else None
            decoder_outputs_repeat = self.decoder(
                input_ids=decoder_input_ids_for_def,
                attention_mask=decoder_attention_mask_for_def,
                inputs_embeds=decoder_inputs_embeds,
                past_key_values=past_key_values,
                encoder_hidden_states=def_hidden_states,
                encoder_attention_mask=def_attention_mask,
                head_mask=decoder_head_mask,
                cross_attn_head_mask=cross_attn_head_mask,
                use_cache=use_cache,
                output_attentions=output_attentions,
            ...
            sequence_output_repeat = decoder_outputs_repeat[0]

            # Set device for model parallelism
            if self.model_parallel:
                torch.cuda.set_device(self.encoder.first_device)
                self.lm_head = self.lm_head.to(self.encoder.first_device)
                sequence_output_repeat = sequence_output_repeat.to(self.lm_head.weight.device)
                # if sequence_output_neg is not None:
                #     sequence_output_neg = sequence_output_neg.to(self.lm_head.weight.device)
            ...
            if self.config.tie_word_embeddings:
                # Rescale output before projecting on vocab
                # See https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/transformer/transformer.py#L586
                sequence_output_repeat = sequence_output_repeat * (self.model_dim**-0.5)
                # if sequence_output_neg is not None:
                #     sequence_output_neg = sequence_output_neg * (self.model_dim**-0.5)

            lm_logits_repeat = self.lm_head(sequence_output_repeat)  ## [batch_size * neg_num , max_seq_length, vocab_size]
            lm_logits_repeat_lis = torch.split(lm_logits_repeat,batch_szie,dim=0)
            lm_logits_repeat = torch.cat(lm_logits_repeat_lis,dim=1)  ## [batch_size , max_seq_length * neg_num, vocab_size]
        ...
        if labels is not None:
            ...
            # ranking with repeated definitions 
            loss_neg = None
            if neg_loss_type is not None:
                if neg_loss_type == "contrastive_loss_repeat":
                    loss_fct_neg = self.contrastive_loss_repeat
                else:
                    raise NotImplementedError("Error neg loss function type!")
                if current_epoch >= main_loss_warm and lm_logits_repeat is not None:
                    # lm_logits_out_neg = lm_logits_out_neg  ## [neg_out_num, max_seq_length, vocab_size]
                    # labels_neg_len_list = [ts.shape[0] for ts in labels_neg]
                    # labels_neg = torch.cat(labels_neg,dim=0) ## [neg_out_num,seq_len]
                    # lm_logits_neg = torch.cat(lm_logits_list[sample_num_pos+1:],dim=1) ## [batch_size,seq_len*3,vocab_size]
                    labels_neg_cons = labels.repeat(1,neg_num) ## [batch_size,seq_len*3]
                    loss_neg = loss_fct_neg(lm_logits_ori,lm_logits_repeat,labels,labels_neg_cons,margin=margin_neg,ignore_index=-100,batch_size=batch_szie,def_len=def_len,max_seq_len=lm_logits.size(1),strategy=strategy)     
                    if torch.isnan(loss_neg).any().item():
                        raise RuntimeError("the loss is nan!")
                    if torch.isinf(loss_neg).any().item():
                        raise RuntimeError("the loss is inf!")
            ...
            if null_loss_type is not None:
                if null_loss_type == "contrastive_loss_max_v2":
                    ...
                    raise NotImplementedError("Error neg loss function type!")
                if current_epoch >= main_loss_warm and lm_logits_null is not None:
                    # lm_logits_out_neg = lm_logits_out_neg  ## [neg_out_num, max_seq_length, vocab_size]
                    # labels_neg_len_list = [ts.shape[0] for ts in labels_neg]
                    # labels_neg = torch.cat(labels_neg,dim=0) ## [neg_out_num,seq_len]
                    # lm_logits_neg = torch.cat(lm_logits_list[sample_num_pos+1:],dim=1) ## [batch_size,seq_len*3,vocab_size]
                    labels_null = labels.repeat(1,null_num) ## [batch_size,seq_len*3]
                    loss_null = loss_fct_null(lm_logits_ori,lm_logits_null,labels,labels_null,margin=margin_null,ignore_index=-100,batch_size=batch_szie)      
            ...
            assert loss_mix_ratio_neg >= 0.0 and loss_mix_ratio_null >= 0.0
        ...
        return Seq2SeqLMOutput(
            ...
            decoder_attentions=decoder_outputs.attentions,
            cross_attentions=decoder_outputs.cross_attentions,
            encoder_last_hidden_state=encoder_outputs.last_hidden_state if return_encoder_outputs else hidden_states,  # this "encoder_last_hidden_state" can not be used 
            encoder_hidden_states=encoder_outputs.hidden_states,
            encoder_attentions=encoder_outputs.attentions,
        )
    ...
    def contrastive_loss_repeat(self,logits_pos,logits_neg,labels_pos,labels_neg,margin=0.5,ignore_index=-100,batch_size=None,strategy="max",max_seq_len:int=None,def_len:list=None):
        ...
        delta = (gt_logits_pos - max_gt_logits) + (margin * logits_mask_pos)  ## TODO: i dont know whether token-wise loss make sense, it is better to use avg logits to calvulate loss (sentence-wise)
        # take the avg of the batch
        # do token-wise loss
        # loss_neg = torch.mean(delta)
        # note the mask
