...
from ni_collator_augment_output_constrain_add_metric import DataCollatorForNI
# from ni_trainer_augment_v2 import NITrainer, DenserEvalCallback
from ni_trainer_argument_output_constrain_add_metric import NITrainer, DenserEvalCallback
...
from modeling_t5_output_constrain_add_metric import T5ForConditionalGeneration_neg
...
def main():
    ...
    raw_datasets = load_dataset(  
        "src/ni_dataset_argument_output_constrain_add_metric.py", 
    ...
    # Metric
    def compute_ni_metrics_new(dataset, preds, preds_cls, save_prefix=None):
        decoded_preds = tokenizer.batch_decode(preds, skip_special_tokens=True)
        decoded_preds_cls = tokenizer.batch_decode(preds_cls, skip_special_tokens=True)
        ...
        results_cls = compute_metrics_add_f(predictions=decoded_preds_cls, references=references)
        # "new" stands for metric calculating on new precition results on cls tasks
        result.update({"exact_match_new": results_cls["exact_match"], "rouge1_new": results_cls["rouge1"] , "rougeL_new": results_cls["rougeL"] })
        ...
        result_per_category_cls = compute_grouped_metrics_add_f_v2(predictions=decoded_preds_cls, references=references, groups=categories)
        em_now = result_per_category_cls['predict_exact_match_avg']
        
        result.update(result_per_category)
        result.update({"predict_exact_match_avg_new":em_now})
    ...
    trainer = NITrainer(
        ...
        compute_metrics=compute_ni_metrics_new if training_args.predict_with_generate else None,
