...
class DataCollatorForNI:
    ...
    def __call__(self, batch, return_tensors=None):
        ...
        for instance in batch:
            ...
            for i,definition in enumerate(definitions):
                ...
                if i == 0:
                    ...
                    # try to append "null" in the input_ids_list
                    if self.add_null:
                        definition = construct_mul_def("NULL.")
                        source = task_name + definition + "".join(pos_examples) + "".join(neg_examples) + task_input
                        tokenized_source = self.tokenizer(source)["input_ids"]
                        # ensure the input length is not too along after encoding
                        # each element in the sources is a string 
                ...
                # for def repeat + x
                    ...
                    definition = definitions[0] + " [REPEAT] " + definition
                    definition = construct_mul_def(definition)
                    source_rp = task_name + definition + "".join(pos_examples) + "".join(neg_examples) + task_input
                    tokenized_source_rp = self.tokenizer(source_rp)["input_ids"]
                    # ensure the input length is not too along after encoding
                    # each element in the sources is a string 
                    if len(tokenized_source_rp) <= self.max_source_length:
                        source_rp_list.append(source_rp)
                    else:
                        source_rp_list.append(self.tokenizer.decode(tokenized_source_rp[:self.max_source_length], skip_special_tokens=True))
                
            
            # assert len(source_list) == 1
            sources.append(source_list)    
            sources_rp.append(source_rp_list)
            source_rp_len.append(len(source_rp_list))
            
        
        # sources: [batch_szie,1,seq_len]
        max_rp_len = max(source_rp_len)
        source_rp_pad = []
        # deal with the length difference of neg def of each instance
        for s_list in sources_rp:
            if len(s_list) < max_rp_len:
                s_list += [""]*(max_rp_len - len(s_list))
            source_rp_pad.append(s_list)
        sources_rp = source_rp_pad
        assert len(sources) == len(sources_rp)  # batch_size
        # assert len(sources[0]) == len(sources_def[0])
        batch_size = len(sources)
        n_num = len(sources[0])
        s_num = len(sources_rp[0])
        ...
        for t in sources:
            source_concat += t 
        source_rp_concat = [] # [batch_szie * (sample_num+2),seq_len]
        for t in sources_rp:
            source_rp_concat += t
        ...
        batch_def_tokenzied = self.tokenizer(
                ...
                truncation=True,
                pad_to_multiple_of=self.pad_to_multiple_of)
        batch_rp_input_ids,batch_rp_attention_mask = batch_def_tokenzied["input_ids"],batch_def_tokenzied["attention_mask"]
        
        # split to a list
        model_inputs = dict()
        input_ids_list = []
        attention_mask_list = []
        for index in range(n_num):  ## TODO: should be more than one def (5 neg def), so the input_list length should be 6
            index_list = [index+t*n_num for t in range(batch_size)]
        ...
        for index in range(s_num):
            ...
            tmp = batch_rp_input_ids[index_list,:]
            def_ids_list.append(tmp)
            tmp_att = batch_rp_attention_mask[index_list,:]
            def_attention_mask_list.append(tmp_att)
        
        model_inputs["input_ids_list"] = input_ids_list # [[batch_size,seq_len]]
        ...
        model_inputs["def_len"] = source_rp_len 
