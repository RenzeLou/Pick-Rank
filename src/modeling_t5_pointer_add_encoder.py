...
class T5ForConditionalGeneration_neg(T5PreTrainedModel):
    ...
    def forward(
        ...
        # Encode if needed (training, first prediction pass)
        if encoder_outputs_ori is None and encoder_outputs_rep is None and encoder_outputs_del is None:
            if not base:
            ...
            # prepare ori def + x
            if (not test_phase and not main_loss_on_rep) or (test_phase and not predict_on_rep):
        ...
        lm_logits_null = None  # TODO: add null ranking to see if it is useful
        ...
        if labels is not None:
            ...
            if neg_loss_type is not None:
                if neg_loss_type == "contrastive_loss_max_v2":
                    ...
                    raise NotImplementedError("Error neg loss function type!")
                if current_epoch >= main_loss_warm and lm_logits_del is not None and lm_logits_ori is not None:
            ...
            if null_loss_type is not None:
                ...
                if current_epoch >= main_loss_warm and lm_logits_null is not None and lm_logits_ori is not None:
                    loss_null = loss_fct_null(lm_logits_ori,lm_logits_null,labels,labels,margin=margin_null,ignore_index=-100,batch_size=batch_size)      
