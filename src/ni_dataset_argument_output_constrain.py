...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_eval_task=None,
                 ...
                 use_first:bool=False,use_last:bool=False,
                 **kwargs):
        ...
        with open(path, encoding="utf-8") as split_f:
            for line in split_f:
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    if not self.config.use_first and not self.config.use_last:
                        # normal setting
                        null_def = []
                        negative_def = []
                        ...
                        assert len(pos_def) == 2
                        if self.config.use_first and self.config.use_last:
                            all_def = [" ".join(pos_def)]
                        elif self.config.use_first:
                            all_def = [pos_def[0]]
