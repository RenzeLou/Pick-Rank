...
class T5ForConditionalGeneration_neg(T5PreTrainedModel):
    ...
    def multiple_forward_batchfy_neg_output_constrain(
        ...
        lm_logits_neg = torch.cat(lm_logits_all_list[1:],dim=1) if neg_num > 0 else None
        ...
        if labels is not None:
            ...
            if neg_loss_type is not None:
                if neg_loss_type == "contrastive_loss_max_v2":
                    ...
                    loss_fct_neg = self.contrastive_loss_max_v4
                elif neg_loss_type == "contrastive_loss_attention":
                    loss_fct_neg = self.contrastive_loss_attention
                ...
                if current_epoch >= main_loss_warm and lm_logits_neg is not None:
                    ...
                    loss_neg = loss_fct_neg(lm_logits_ori,lm_logits_neg,labels,labels_neg_cons,margin=margin_neg,ignore_index=-100,weights=weights,batch_size=batch_szie)      
                    if torch.isnan(loss_neg).any().item():
                        raise RuntimeError("the loss is nan!")
                    if torch.isinf(loss_neg).any().item():
                        raise RuntimeError("the loss is inf!")
