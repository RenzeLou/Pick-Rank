...
"""
this brach of files are used for the 'silver output examples' experiments
that is, add the "silver_output" in the "Positive examples", and then use model to predict the performance
the silver output can be generated from:
1. pretrained model on the training set (def+x -> y)
2. adv training model
"""
...
from ni_collator_augment_silver_output import DataCollatorForNI
# from ni_trainer_augment_v2 import NITrainer, DenserEvalCallback
from ni_trainer_example_silver_output import NITrainer, DenserEvalCallback
...
from modeling_t5_example_silver_output import T5ForConditionalGeneration_neg
...
class DataTrainingArguments:
    ...
    silver_y: Optional[bool] = field(
        default=False,
        metadata={"help": "use the silver y in the examples."}
    )
    shuffle_y: Optional[bool] = field(
        default=False,
        metadata={"help": "shuffle the order of y in the examples."}
    )
    silver_x_y: Optional[bool] = field(
        default=False,
        metadata={"help": "use the silver (x,y) pairs as the examples."}
    )
    random_select_pos: Optional[bool] = field(
        default=False,
        metadata={"help": "random select the pos examples instead of the frst two examples."}
    )
    num_pos_examples: Optional[int] = field(
        default=0,
        metadata={"help": "number of in-context positive examples."}
...
def main():
    ...
    raw_datasets = load_dataset(
        "src/ni_dataset_example_silver_out.py", 
    ...
    data_collator = DataCollatorForNI(
        ...
        silver_y=data_args.silver_y,
        silver_x_y=data_args.silver_x_y,
        shuffle_y=data_args.shuffle_y,
        random_select_pos=data_args.random_select_pos
    )
