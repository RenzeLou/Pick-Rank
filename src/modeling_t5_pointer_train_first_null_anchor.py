...
class T5ForConditionalGeneration_neg(T5PreTrainedModel):
    ...
    def multiple_forward_batchfy(
        ...
        lm_logits_list = torch.split(lm_logits,batch_size,dim=0)
        ...
        if labels is not None:
            ...
            if sample_num_neg > 0 and current_epoch >= main_loss_warm:
                ...
                loss_neg = loss_fct_neg(lm_logits_ori,lm_logits_neg,labels,labels_neg,margin=margin_neg,ignore_index=-100,batch_size=batch_size)      
            ...
            if sample_num_pos > 0 and current_epoch >= main_loss_warm:
                ...
                loss_pos = loss_fct_pos(lm_logits_ori,lm_logits_pos,labels,labels_pos,sample_num=sample_num_pos,margin=margin_pos,ignore_index=-100,batch_size=batch_size)      
            # overall loss
            loss_addition = 0.
    ...
    def forward(
        ...
        ''' calculate the ranking loss for the given batch '''
        if ori_def_list is None and x_tk_list is None and def_input_ids_list is None and def_attention_mask_list is None:
            ...
            # labels = self.gen_kwargs.get("labels", None) # do not calculate the loss when doing test to save time
        ...
        neg_num = len(def_input_ids_list) ## 3 (individual sentences)
        ...
        if encoder_outputs_ori is None and encoder_outputs_rep is None and encoder_outputs_del is None and encoder_outputs_null is None:
            if not base:
                ...
                if not test_phase and (null_loss_type is not None or neg_loss_type is not None or pos_loss_type is not None):
                    def_null_list = []
            ...
            # prepare ori def + x
            if (not test_phase and (neg_loss_type is not None or not main_loss_on_rep)) or (test_phase and not predict_on_rep):
        ...
        # elif return_dict and not isinstance(encoder_outputs_ori, BaseModelOutput):
        else:
        ...
        if labels is not None:
            ...
            if pos_loss_type is not None:
                if pos_loss_type == "contrastive_loss_repeat":
                    ...
                    raise NotImplementedError("Error neg loss function type!")
                if current_epoch >= main_loss_warm and lm_logits_rep is not None and lm_logits_null is not None:
                    def_len_for_loss = [1] * batch_size
                    loss_pos = loss_fct_pos(lm_logits_null,lm_logits_rep,labels,labels,margin=margin_pos,ignore_index=-100,batch_size=batch_size,def_len=def_len_for_loss,max_seq_len=lm_logits_ori.size(1))     
            ...
            if neg_loss_type is not None:
                if neg_loss_type == "contrastive_loss_max_v2":
                    ...
                    raise NotImplementedError("Error neg loss function type!")
                if current_epoch >= main_loss_warm and lm_logits_null is not None and lm_logits_ori is not None:
                    # loss_neg = loss_fct_neg(lm_logits_ori,lm_logits_del,labels,labels,margin=margin_neg,ignore_index=-100,batch_size=batch_size)     
                    loss_neg = loss_fct_neg(lm_logits_ori,lm_logits_null,labels,labels,margin=margin_neg,ignore_index=-100,batch_size=batch_size)     
                    if torch.isnan(loss_neg).any().item():
                        raise RuntimeError("the loss is nan!")
                    if torch.isinf(loss_neg).any().item():
                        raise RuntimeError("the loss is inf!")
            ...
            if null_loss_type is not None:
                if null_loss_type == "contrastive_loss_max_v2":
                    ...
                    raise NotImplementedError("Error neg loss function type!")
                if current_epoch >= main_loss_warm and lm_logits_null is not None and lm_logits_del is not None:
                    loss_null = loss_fct_null(lm_logits_del,lm_logits_null,labels,labels,margin=margin_null,ignore_index=-100,batch_size=batch_size)      
            ...
            # TODO: check torch.autograd.grad(loss,list(self.pointer_encoder.parameters())[0],retain_graph=True)
            # TODO: check torch.autograd.grad(loss,list(self.encoder.parameters())[0],retain_graph=True)
            loss = loss_ori + loss_addition  # check if the loss can be backpropagated to the pointer network # torch.autograd.grad(loss,next(self.pointer_projector.parameters()),retain_graph=True)
        ...
        ''' 6. update past_key_values for the next step (testing) '''
        if test_phase:
