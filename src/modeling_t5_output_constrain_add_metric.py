...
class T5ForConditionalGeneration_neg(T5PreTrainedModel):
    ...
    def multiple_forward_batchfy_neg_output_constrain(
        self,
        input_ids_list: Optional[List[torch.LongTensor]] = None,
        attention_mask_list: Optional[List[torch.FloatTensor]] = None,
        decoder_input_ids: Optional[torch.LongTensor] = None,
        ...
        main_loss_warm: Optional[int] = 0,
        current_epoch: Optional[int] = 0,
        label_space_ids: Optional[torch.LongTensor] = None,
        label_space_ids_len: Optional[List[int]] = None,
        label_space_decoder_input_ids: Optional[torch.LongTensor] = None,
    ) -> Union[Tuple[torch.FloatTensor], Seq2SeqLMOutput]:
        '''
        only calculate the crossentropy loss on the inputs
        '''
        ...
        if labels is not None and decoder_input_ids is None and decoder_inputs_embeds is None:
            # get decoder inputs from shifting lm labels to the right
            decoder_input_ids = self._shift_right(labels)
        if labels_neg is not None and decoder_input_ids_neg is None and decoder_inputs_embeds is None:
            # get decoder inputs from shifting lm labels to the right
            decoder_input_ids_neg = self._shift_right(labels_neg)
        ...
        ''' in order to avoid empty tensor '''
        if decoder_input_ids_neg is not None:
            neg_out_num = decoder_input_ids_neg.shape[0]
            if neg_out_num == 0:
                decoder_input_ids_neg = None
        else:
            neg_out_num = 0
        
        sequence_output_neg = None
        if decoder_input_ids_neg is not None:
            labels_neg_len_tensor = torch.tensor(labels_neg_len).cuda(hidden_states.device)
            hidden_states_rep = torch.repeat_interleave(hidden_states,labels_neg_len_tensor,dim=0)
            attention_mask_rep = torch.repeat_interleave(attention_mask,labels_neg_len_tensor,dim=0)
            # try:
            decoder_outputs_neg = self.decoder(
                input_ids=decoder_input_ids_neg,
                attention_mask=decoder_attention_mask,
                inputs_embeds=decoder_inputs_embeds,
                past_key_values=past_key_values,
                encoder_hidden_states=hidden_states_rep,
                encoder_attention_mask=attention_mask_rep,
                head_mask=decoder_head_mask,
                cross_attn_head_mask=cross_attn_head_mask,
                use_cache=use_cache,
                output_attentions=output_attentions,
            ...
            # except RuntimeError:
            #     wait = True
            sequence_output_neg = decoder_outputs_neg[0]
        ...
        lm_logits_ori = self.lm_head(sequence_output)  ## [batch_size, max_seq_length, vocab_size]
        lm_logits_neg = self.lm_head(sequence_output_neg) if sequence_output_neg is not None else None ## [neg_out_num, max_seq_length, vocab_size]
        # lm_logits_list = torch.split(lm_logits,batch_szie,dim=0)
        
        loss = None
        if labels is not None:
            loss_fct_ori = CrossEntropyLoss(ignore_index=-100)
            # assert len(lm_logits_list) == 1, "in this experiment, we will not use any additional pos/neg def again!"
            
            # ori loss, maximazing the ground truth probability
            loss_ori = 0
            # lm_logits_ori = lm_logits  ## [batch_size,seq_len,vocab_size]
            if not neg_loss_only:
                loss_ori = loss_fct_ori(lm_logits_ori.view(-1, lm_logits_ori.size(-1)), labels.view(-1))
            
            # neg loss (contrastive loss, output constrain) 
            loss_neg = None
            if neg_loss_type == "contrastive_loss_out_constrain_all":
                loss_fct_neg = self.contrastive_loss_out_constrain_all
            else:
                raise NotImplementedError("Error neg loss function type!")
            if current_epoch >= main_loss_warm and lm_logits_neg is not None:
                # lm_logits_neg = lm_logits_neg  ## [neg_out_num, max_seq_length, vocab_size]
                # labels_neg_len_list = [ts.shape[0] for ts in labels_neg]
                # labels_neg = torch.cat(labels_neg,dim=0) ## [neg_out_num,seq_len]
                loss_neg = loss_fct_neg(lm_logits_ori,labels,lm_logits_neg,labels_neg,labels_neg_len_tensor,labels_neg_len,margin=margin_neg,ignore_index=-100,batch_size=batch_szie)      
                if torch.isnan(loss_neg).any().item():
                    raise RuntimeError("the loss is nan!")
                if torch.isinf(loss_neg).any().item():
                    raise RuntimeError("the loss is inf!")
            ...
            # overall loss
            loss_addition = 0.
            loss_addition += loss_neg.clamp(min=0.0) if loss_neg is not None else 0.
            assert loss_mix_ratio >= 0.0
            loss = loss_ori + loss_addition * loss_mix_ratio
    ...
    def multiple_forward_batchfy_neg_output_constrain_eval_with_ground_truth(
        self,
        input_ids_list: Optional[List[torch.LongTensor]] = None,
        attention_mask_list: Optional[List[torch.FloatTensor]] = None,
        decoder_input_ids: Optional[torch.LongTensor] = None,
        ...
        '''
        use teacher forcing to get the final logits of the model corresponding to the label space
        then get the probability of each label in the label space
        select the one with the highest probability as the final prediction
        '''
        use_cache = use_cache if use_cache is not None else self.config.use_cache
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        ...
        # else:
        #     raise ValueError
        batch_size = len(label_space_ids_len)
        ...
        if self.model_parallel:
            torch.cuda.set_device(self.decoder.first_device)
            hidden_states = hidden_states.to(self.decoder.first_device)
            if label_space_decoder_input_ids is not None:
                label_space_decoder_input_ids = label_space_decoder_input_ids.to(self.decoder.first_device)
            if attention_mask is not None:
                attention_mask = attention_mask.to(self.decoder.first_device)
        ...
        ''' in order to avoid empty tensor (i.e., all generation instance in this batch)'''
        if label_space_decoder_input_ids is not None:
            all_label_num = label_space_decoder_input_ids.shape[0]
            if all_label_num == 0:
                label_space_decoder_input_ids = None
        else:
            all_label_num = 0
        
        sequence_output = None
        if label_space_decoder_input_ids is not None:
            label_space_ids_len_tensor = torch.tensor(label_space_ids_len).cuda(hidden_states.device)
            hidden_states_rep = torch.repeat_interleave(hidden_states,label_space_ids_len_tensor,dim=0)
            attention_mask_rep = torch.repeat_interleave(attention_mask,label_space_ids_len_tensor,dim=0)
            # try:
            decoder_outputs = self.decoder(
                input_ids=label_space_decoder_input_ids,
                attention_mask=None,
                inputs_embeds=None,
                past_key_values=past_key_values,
                encoder_hidden_states=hidden_states_rep,
            ...
            # except RuntimeError:
            #     wait = True
            sequence_output = decoder_outputs[0]  ## [all_label_num, seq_len, 768]
        ...
        if self.config.tie_word_embeddings and sequence_output is not None:
            # Rescale output before projecting on vocab
            # See https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/transformer/transformer.py#L586
            sequence_output = sequence_output * (self.model_dim**-0.5)
            # if sequence_output_neg is not None:
        ...
        lm_logits = self.lm_head(sequence_output) if sequence_output is not None else None  ## [all_label_num, max_seq_length, vocab_size]
        
        # generate the prediction according to the label space (i.e., candidate)
        prediction = []
        if lm_logits is not None:
            gt_logits,logits_mask = self.get_logits(lm_logits,label_space_ids,ignore_index=-100,return_mask=True)  ## [all_label_num, max_seq_length,1]
            # take the avg of ground truth token probability
            sum_gt_logits = torch.sum(gt_logits,dim=1) ## [all_label_num, 1]
            sum_logits_mask = torch.sum(logits_mask,dim=1)
            avg_gt_logits = torch.div(sum_gt_logits,sum_logits_mask) ## [all_label_num, 1]
            # split the answer probability of current batch, get the answer distribution of each instance
            label_distribution = torch.split(avg_gt_logits,label_space_ids_len,dim=0)
            label_space = torch.split(label_space_ids,label_space_ids_len,dim=0)
            seq_len  = label_space_ids.shape[1]
            for lb_dis,lb_sp in zip(label_distribution,label_space):
                lb_dis = lb_dis.squeeze()
                choice = torch.argmax(lb_dis)
                if lb_sp.shape[0] > 0:
                    prediction.append(lb_sp[choice,:].unsqueeze(0))
                else:
                    # for those instances in the gen tasks, just pad the results 
                    prediction.append((torch.ones(seq_len)*-100).type_as(lb_sp).unsqueeze(0))
            assert len(prediction) == batch_size
        
        prediction_batch = torch.cat(prediction) if len(prediction) > 0 else None # [batch_size, seq_len]
    ...
    def contrastive_loss_max_v4(self,logits_pos,logits_neg,labels_pos,labels_neg,margin=0.5,ignore_index=-100,batch_size=None):
        ...
        # calculate the loss: N - P + margin
        # be aware that the margin shoud also be masked
        delta = (max_gt_logits - gt_logits_pos) + (margin * logits_mask_pos)  ## TODO: set the margin to 0.3 temporally according to the observation
        # take the avg of the batch
        # do token-wise loss
        # loss_neg = torch.mean(delta)
        # note the mask
    ...
    def contrastive_loss_all(self,logits_pos,logits_neg,labels_pos,labels_neg,margin=0.5,ignore_index=-100,batch_size=None):
        ...
        # calculate the loss: N - P + margin
        # be aware that the margin shoud also be masked
        # delta = (max_gt_logits - gt_logits_pos) + (margin * logits_mask_pos.squeeze())  
        # take the avg of the batch
        # assert len(delta.shape) == 2, 'no way?'
        # if batch_size == 1:
        #     loss_neg = torch.sum(delta)
