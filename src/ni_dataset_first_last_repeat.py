...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_eval_task=None,
                 ...
                 use_first:bool=False,use_last:bool=False,use_first_last_random:bool=False,
                 **kwargs):
        ...
        self.use_first_last_random = use_first_last_random
        ...
        with open(path, encoding="utf-8") as split_f:
            for line in split_f:
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    if not self.config.use_first and not self.config.use_last and not self.config.use_first_last_random:
                        null_def = []
                        negative_def = []
                        if self.config.null_loss_type is not None:
                            null_def = ["NULL."]
                        if self.config.neg_loss_type is not None:
                            negative_def = neg_def_sampled + pos_def_sampled
                        # concatnate ori def, null def and neg def
                        all_def = ori_def + null_def + negative_def
                    else:
                        if self.config.use_first:
                            all_def = ori_def + [pos_def[0]]
                        elif self.config.use_last:
                            all_def = ori_def + [pos_def[-1]]
                        elif self.config.use_first_last_random:
                            all_def = ori_def + [random.choice(pos_def)]
                        assert len(all_def) == 2
