...
class NITrainer(Seq2SeqTrainer):
    ...
    def init_hyper(self,pos_neg_ratio:float,margin_pos:float,margin_neg:float,neg_loss_type:str,loss_mix_ratio:float,sample_num_neg:int,sample_num_pos:int,main_loss_warm:int):
        self.pos_neg_ratio = pos_neg_ratio
        self.margin_pos = margin_pos
        self.margin_neg = margin_neg
        self.neg_loss_type = neg_loss_type
        self.loss_mix_ratio = loss_mix_ratio
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits,logits_cls, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            ...
            if logits_cls is not None:
                logits_cls = self._pad_across_processes(logits_cls)
                logits_cls = self._nested_gather(logits_cls)
                if self.preprocess_logits_for_metrics is not None:
                    logits_cls = self.preprocess_logits_for_metrics(logits_cls, labels)
                preds_host_cls = logits_cls if preds_host_cls is None else nested_concat(preds_host_cls, logits_cls, padding_index=-100)
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                if preds_host_cls is not None:
                    logits_cls = nested_numpify(preds_host_cls)
                    all_preds_cls = logits_cls if all_preds_cls is None else nested_concat(all_preds_cls, logits_cls, padding_index=-100)
                ...
                losses_host, preds_host,preds_host_cls, labels_host = None, None, None, None
        ...
        if all_preds_cls is not None:
            all_preds_cls = nested_truncate(all_preds_cls, num_samples)
        if all_labels is not None:
            all_labels = nested_truncate(all_labels, num_samples)

        # Metrics!
        if self.compute_metrics is not None and all_preds is not None and all_labels is not None:
            metrics = self.compute_metrics(dataset=eval_dataset, preds=all_preds, preds_cls=all_preds_cls, save_prefix=metric_key_prefix)
        else:
            metrics = {}
    ...
    def prediction_step(
        ...
        assert len(inputs["input_ids_list"]) == len(inputs["attention_mask_list"]) == 1, "when doing test, the length of input list == 1 because there is no neg instruction"
        ...
        with torch.no_grad():
            ...
            #         outputs,predictions = model.multiple_forward_batchfy_neg_output_constrain_eval_with_ground_truth(**inputs)
            ...
            # else:
            #     loss = (outputs["loss"] if isinstance(outputs, dict) else outputs[0]).mean().detach()
            loss = None # we dont care the loss when doing eval # TODO: implement loss calculation 
        ...
        return (loss, generated_tokens,predictions,labels)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # inputs["pos_neg_ratio"] = self.pos_neg_ratio
        ...
        inputs["sample_num_pos"] = 0
        inputs["sample_num_neg"] = 0
        inputs["main_loss_warm"] = self.main_loss_warm
        inputs["current_epoch"] = self.state.epoch
