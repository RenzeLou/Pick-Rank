...
from ni_collator_augment_v4 import DataCollatorForNI
# from ni_trainer_augment_v2 import NITrainer, DenserEvalCallback
from ni_trainer_augment_v4 import NITrainer, DenserEvalCallback
...
from modeling_t5_mod_v2 import T5ForConditionalGeneration_neg
...
def main():
    ...
    raw_datasets = load_dataset(
        "src/ni_dataset_argument_silver_output_for_examples.py", 
    ...
    if training_args.do_predict:
        # predict the silver labels for examples on both train and test
        if "train" not in raw_datasets:
            raise ValueError("requires a train dataset")
        predict_dataset = raw_datasets["train"]
        if data_args.max_train_samples is not None:
            raise NotImplementedError("dont care")
            predict_dataset = predict_dataset.select(range(data_args.max_train_samples))
        
        if "test" not in raw_datasets:
            raise ValueError("requires a test dataset")
        predict_dataset_test = raw_datasets["test"]
        if data_args.max_train_samples is not None:
            raise NotImplementedError("dont care")
            predict_dataset = predict_dataset.select(range(data_args.max_train_samples))
    ...
    #         predict_dataset = predict_dataset_test.select(range(data_args.max_predict_samples))
    ...
    trainer.init_hyper(pos_neg_ratio=training_args.pos_neg_ratio,margin_pos=training_args.margin_pos,
                       ...
                       loss_mix_ratio=training_args.loss_mix_ratio,sample_num_neg=data_args.sample_num_neg,
    ...
    if training_args.do_predict:
        assert training_args.predict_with_generate, "need to generate the output"
        logger.info("*** Predict on Training set***")
        ...
        predict_results_test = trainer.predict(
            predict_dataset_test, metric_key_prefix="predict_test", max_length=max_length, num_beams=num_beams
        )
        metrics_test = predict_results_test.metrics
        max_predict_samples_test = (
            data_args.max_predict_samples if data_args.max_predict_samples is not None else len(predict_dataset_test)
        )
        metrics_test["predict_samples_test"] = min(max_predict_samples_test, len(predict_dataset_test))

        trainer.log(metrics_test)
        trainer.log_metrics("predict_test", metrics_test)
        trainer.save_metrics("predict_test", metrics_test)

        all_metrics.update(metrics_test)

        assert trainer.is_world_process_zero(), "need to generate output"
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                predictions_test = tokenizer.batch_decode(
                    predict_results_test.predictions, skip_special_tokens=True, clean_up_tokenization_spaces=True
                )
                predictions_test = [pred.strip() for pred in predictions_test]
                # output_prediction_file = os.path.join(training_args.output_dir, "generated_predictions.txt")
                # with open(output_prediction_file, "w") as writer:
                #     writer.write("\n".join(predictions))
                output_prediction_file = os.path.join(training_args.output_dir, "predicted_examples_test.jsonl")
                with open(output_prediction_file, "w") as fout:
                    for example, prediction in zip(predict_dataset_test, predictions_test):
                        example["prediction"] = prediction
                        fout.write(json.dumps(example) + "\n")
