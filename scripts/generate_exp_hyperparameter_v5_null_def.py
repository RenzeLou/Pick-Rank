...
loss_mix_ratio_list = [0.001,0.003,0.01,0.03,0.1,0.3,1]  # 0.1,0.3,0.6
# loss_mix_ratio_list_already = []
margin_pos_list = [0] # 0.0001,0.001,0.1
margin_neg_list = [0.001,0.003,0.006,0.01,0.03,0.06,0.1]
sample_num_pos = [0]
sample_num_neg = [0]
...
for ratio in loss_mix_ratio_list:
    shell_name = "null_def_cons-ratio_{}".format(ratio)
# for lr in lr_list:
...
'''
    ...
    template = template.format(model_name,margin_pos_list[0],sample_num_pos[0],sample_num_neg[0],pos_neg_ratio_list[0],ratio,lr_list[0])

    # ======== large scale experiments (tuning parameters) =========
    t = 'export margin_neg="{}"'
    same = '''
export EXP="ratio_${loss_mix_ratio}-margin_${margin_neg}"
...
deepspeed --master_port $port  src/run_s2s_test_pos.py \\
    ...
    --output_dir output_null_def_cons/ \\
    --cache_dir ./cache/ \\
    --overwrite_cache \\
