...
model_name = "t5-base"
sample_num_list = [1]  ## 1,2,3,4,5,6 ## there are 5 kinds of negative instructions
# sample_num = 1
...
margin_pos_list = [0.0003]
margin_neg_list = [0.001]
...
# neg_loss_only = False  
pos_neg_ratio_list = [0.1,0.3,0.6,0.9] # 0,0.5,1
...
for ratio in pos_neg_ratio_list :
    shell_name = "new_add_pos_ratio-{}".format(ratio)
...
'''
    ...
    template = template.format(model_name,margin_pos_list[0],margin_neg_list[0],sample_num_list[0],loss_mix_ratio_list[0],ratio)

    # ======== large scale experiments (tuning parameters) =========
    same = '''
export EXP="new-pos_neg_${pos_neg_ratio}-mix_ratio_${loss_mix_ratio}-margin_pos_${margin_pos}-margin_neg_${margin_neg}-num_${sample_num}"
...
deepspeed --master_port $port  src/run_s2s_test_pos.py \\
    ...
    --task_dir data/tasks/pos_neg_mixed_v2 \\
    --output_dir output/ \\
    --cache_dir ./cache/ \\
    --overwrite_cache \\
