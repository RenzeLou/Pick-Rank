...
in every script, there are a series of experiments to find a correct ratio for the corresponding sample_num,
that is, each script contains the experiments with one fixed sample_num and several different ratios
"""
...
for sample_num in sample_num_list:
    shell_name = "{}-search_sample_num_{}.sh".format(model_name,sample_num)
...
'''
    ...
    template = template.format(model_name,sample_num)

    # ======== large scale experiments (tuning parameters) =========
    t = 'export loss_mix_ratio="{}"'
    same = '''
export EXP="${model}-sample-${sample_num}-ratio-${loss_mix_ratio}"
...
deepspeed --master_port $port  src/run_s2s_test.py \\
    ...
    for loss_mix_ratio in loss_mix_ratio_list:
        template += t.format(loss_mix_ratio) + same + "\n"
