...
class T5ForConditionalGeneration_neg(T5PreTrainedModel):
    ...
    def forward(
        ...
        # get the probability of the labels
        if get_prob:
            labels_prob,prob_mask = self.get_logits(lm_logits, labels,return_mask=True) ## [batch_size,seq_len,1]
            return lm_logits,labels_prob,prob_mask
    ...
    def multiple_forward_batchfy_neg_output_constrain(
        ...
        if neg_loss_type is not None:
            neg_num = sample_num_neg + sample_num_pos
        else: 
            neg_num = 0
        ...
        ## TODO: should make sure
        assert sample_num == 1 + null_num + neg_num, "should be 1 (ori) + 1 (null) + 5 (neg)"
        ...
        if decoder_input_ids_neg is not None:
            ...
            # hidden_states_rep = torch.repeat_interleave(hidden_states,labels_neg_len_tensor,dim=0)  ## TODO: test batch_size == 1 and no null and no neg
            hidden_states_ori = hidden_states[:batch_szie]
            attention_mask_ori = attention_mask[:batch_szie]
            hidden_states_rep = torch.repeat_interleave(hidden_states_ori,labels_neg_len_tensor,dim=0)
            attention_mask_rep = torch.repeat_interleave(attention_mask_ori,labels_neg_len_tensor,dim=0)
            # try:
        ...
        lm_logits = self.lm_head(sequence_output)  ## [batch_size * 7, max_seq_length, vocab_size]
        lm_logits_all_list = torch.split(lm_logits,batch_szie,dim=0)
        assert len(lm_logits_all_list) == sample_num
        lm_logits_ori = lm_logits_all_list[0]
        lm_logits_null = torch.cat(lm_logits_all_list[1:1+null_num],dim=1) if null_num > 0 else None
        lm_logits_neg = torch.cat(lm_logits_all_list[1+null_num:1+null_num+neg_num],dim=1) if neg_num > 0 else None
        lm_logits_out_neg = self.lm_head(sequence_output_neg) if sequence_output_neg is not None else None ## [neg_out_num, max_seq_length, vocab_size]
        # lm_logits_list = torch.split(lm_logits,batch_szie,dim=0)
        ...
        if labels is not None:
            ...
            if neg_loss_type is not None:
                ...
                if current_epoch >= main_loss_warm and lm_logits_neg is not None:
                    ...
                    loss_neg = loss_fct_neg(lm_logits_ori,lm_logits_neg,labels,labels_neg_cons,margin=margin_neg,ignore_index=-100,batch_size=batch_szie)      
                    if torch.isnan(loss_neg).any().item():
                        raise RuntimeError("the loss is nan!")
                    if torch.isinf(loss_neg).any().item():
                        raise RuntimeError("the loss is inf!")
            # out loss 
            loss_out = None
            if out_loss_type is not None:
                if out_loss_type == "contrastive_loss_out_constrain_all":
                    loss_fct_out = self.contrastive_loss_out_constrain_all
                else:
                    raise NotImplementedError("Error neg loss function type!")
                if current_epoch >= main_loss_warm and lm_logits_out_neg is not None:
                    # lm_logits_out_neg = lm_logits_out_neg  ## [neg_out_num, max_seq_length, vocab_size]
                    # labels_neg_len_list = [ts.shape[0] for ts in labels_neg]
                    # labels_neg = torch.cat(labels_neg,dim=0) ## [neg_out_num,seq_len]
                    loss_out = loss_fct_out(lm_logits_ori,labels,lm_logits_out_neg,labels_neg,
                                            labels_neg_len_tensor,labels_neg_len,margin=margin_out,
                                            ignore_index=-100,batch_size=batch_szie)      
                    if torch.isnan(loss_out).any().item():
                        raise RuntimeError("the loss is nan!")
                    if torch.isinf(loss_out).any().item():
                        raise RuntimeError("the loss is inf!")
            ...
            loss_addition += loss_out.clamp(min=0.0) * loss_mix_ratio_out if loss_out is not None else 0.
            
            assert loss_mix_ratio_neg >= 0.0 and loss_mix_ratio_null >= 0.0 and loss_mix_ratio_out >= 0.0
